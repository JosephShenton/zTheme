...
# Set Theme Bundle ID
if [[ "$hasBundle" = false ]]; then
    read -p "Theme Bundle ID ($defaultBundle): " themeBundle
    themeBundle=${themeBundle:-"$defaultBundle"}
fi

# Set Theme URL
if [[ "$hasURL" = false ]]; then
    read -p "Theme URL ($defaultURL): " themeURL
    themeURL=${themeURL:-"$defaultURL"}
fi

# Set iOS Theme Location
if [[ "$hasDEB" = false ]]; then
    read -p "iOS Theme DEB Location (REQUIRED): " iOSTheme
    iOSTheme=$iOSTheme
fi

# Set Uses Features
if [[ "$hasFeatures" = false ]]; then
    read -p "Uses Features (false) (true/false): " usesFeatures
...
read -p "Would you like to copy your theme to the ThemeKit folder? " -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]
then
    yes | cp -rf "/Users/$USER/Desktop/zTheme/$themeName.bundle" "/Users/$USER/ThemeKit/$themeName.bundle"
    echo "Done! Opening ThemeKit"
    open /Applications/ThemeKit.app/
fi